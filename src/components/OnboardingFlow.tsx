...
function SwipeButton({ label, onComplete, hidden }: { label: string; onComplete?: () => void; hidden?: boolean }) {
  ...
  const handlePointerDown = (e: React.PointerEvent) => {
    ...
    // 드래그가 시작되면 힌트 애니메이션(transform)이 실제 이동값과 겹쳐 싸우지 않도록 즉시 끈다.
    iconRef.current?.classList.remove("animate-swipe-hint");
  };
  ...
  const handlePointerUp = (e: React.PointerEvent) => {
    ...
    if (currentX.current > max * 0.8) {
      ...
      currentX.current = 0;
      updateDOM(0, true);
      // 되돌아가는 0.3s 트랜지션이 끝난 뒤에 힌트를 다시 켜서 트랜지션과 겹치지 않게 한다.
      window.setTimeout(() => {
        iconRef.current?.classList.add("animate-swipe-hint");
      }, 320);
    }
  };
  ...
  return (
    <div
      className="relative -mb-1 w-full h-16.5 [@media(max-height:700px)]:h-13.5 shrink-0 flex items-center justify-center overflow-hidden rounded-[50px] bg-[linear-gradient(135deg,rgba(255,255,255,0.12)_0%,rgba(255,255,255,0.03)_100%)] border border-white/18 shadow-[0_8px_32px_0_rgba(0,0,0,0.25),inset_0_1px_1px_rgba(255,255,255,0.1)] cursor-pointer"
      ref={containerRef}
      aria-hidden={hidden || undefined}
      style={hidden ? { visibility: "hidden", pointerEvents: "none" } : undefined}
    >
      {/* 노브가 지나간 만큼 lime으로 차오르는 진행률 채움 */}
      <span
        className="absolute inset-y-0 left-0 w-0 rounded-full bg-[#D6FF1E] pointer-events-none"
        ref={fillRef}
        aria-hidden
      />
      <span
        className="animate-swipe-hint absolute left-1 top-1/2 -translate-y-1/2 w-14.5 h-14.5 [@media(max-height:700px)]:w-11.5 [@media(max-height:700px)]:h-11.5 shrink-0 rounded-full bg-primary-lime flex items-center justify-center"
        ref={iconRef}
        aria-hidden
...
export default function OnboardingFlow({ onComplete }: { onComplete?: () => void }) {
  ...
  return (
    ...
    <div className="self-start w-full max-w-[var(--frame-width)] h-full mx-auto bg-[var(--bg-app)] flex flex-col">
      ...
      <div className="flex flex-col items-center gap-5.5 [@media(max-height:700px)]:gap-2.5 pt-5.5 [@media(max-height:700px)]:pt-3 px-(--gutter) pb-3.5 [@media(max-height:700px)]:pb-2">
        <SwipeButton
        ...
        <div className="w-33.5 h-1.25 rounded-[3px] bg-white" />
      </div>
    </div>
  );
}
